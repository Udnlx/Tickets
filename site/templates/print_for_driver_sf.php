...
$selected_bus = !empty($_POST['print_selected_bus_sf'])?$_POST['print_selected_bus_sf']:NULL;  
$selected_id_bus = !empty($_POST['print_selected_id_bus_sf'])?$_POST['print_selected_id_bus_sf']:NULL;
$selected_date = !empty($_POST['print_selected_date_sf'])?$_POST['print_selected_date_sf']:NULL;
$selected_time = !empty($_POST['print_selected_time_sf'])?$_POST['print_selected_time_sf']:NULL;
...
foreach ($reserv_seat as $reserv_seat_item) {
    $arr_reserv_seat[] = array(
        'name_station_finish' => $reserv_seat_item->name_station_finish,
        ...
        <table style="width: 100%; text-align:center;">
            <thead>
                <tr>
                    ...
                    <th style="line-height: 0.8; font-size: 10px; width: 7%;">Статус</th>
                    <th style="line-height: 0.8; font-size: 10px; width: 12%;">Станция высадки</th>
                    <th style="line-height: 0.8; font-size: 10px; width: 7%;">Сумма<br>к оплате</th>
                    <th style="line-height: 0.8; font-size: 10px; width: 20%;">Пассажир</th>
                    ...
                    <th style="line-height: 0.8; font-size: 10px; width: 15%;">Комментарий</th>
                </tr>
            </thead>
            <tbody>
    ';
            ...
            if ($agent_ticket != 'Донбасс АВ') {
                ...
                $list .= '
                    <tr>
                        <td style="padding: 0px 10px;">' . $val['seat'] . '</td>
                        <td style="padding: 0px 10px;">' . $val['pay_or_booking'] . '</td>
                        <td style="padding: 0px 10px;font-size: 10px;">' . $val['name_station_finish'] . '</td>
                        <td style="padding: 0px 10px;">' . $booking_sum . '</td>
                        <td style="padding: 0px 10px;">' . $val['passenger'] . '</td>
                        <td style="padding: 0px 10px;">' . $val['type_ticket'] . '</td>
                        <td style="padding: 0px 10px;">' . $page_passenger->birthday_passenger . '</td>
