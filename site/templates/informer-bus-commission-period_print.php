...
<div id="content" style="max-width: 700px;">
	<h1 class="uk-heading-hero uk-text-center">Печать отчета по продаже комиссия за период</h1>
...
foreach ($all_agent_tickets as $all_agent_tickets_item) {
    ...
    $prepayment = (int) $all_agent_tickets_item->booking_sum;
    $remains = $price_ticket - $commission - $prepayment;

    $sum_predoplata = $sum_predoplata + $prepayment;
...
$title = array
(
'Отчет по продаже комиссия по маршруту ' . $selected_bus . 'и агенту ' . $agent . ' за период с ' . $start_date . ' по ' . $finish_date,
'',
);
...
header(sprintf( 'Content-Disposition: attachment; filename=Отчет по продаже комиссия по маршруту ' . $selected_bus . ' и агенту ' . $agent . ' - %s.csv', date( 'dmY-His' ) ) );
